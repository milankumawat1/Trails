import im from '../../Assets/card-image.jpg'
import singapore1 from '../../Assets/pdf/singapore/singaporewithuniversalstudio.pdf'
import singapore2 from '../../Assets/pdf/singapore/Singapore 2.pdf'
import singapore3 from '../../Assets/pdf/singapore/Singapore 3.pdf'
import singapore4 from '../../Assets/pdf/singapore/Singapore 4.pdf'
import s1 from '../../Assets/pdf/singapore/singapore1.webp'
import s2 from '../../Assets/pdf/singapore/s2.webp'
import s3 from '../../Assets/pdf/singapore/s3.webp'
import s4 from '../../Assets/pdf/singapore/s4.webp'
import Dubai1 from '../../Assets/pdf/Dubai/Dubai 1.pdf'
import Dubai2 from '../../Assets/pdf/Dubai/Dubai 2.pdf'
import Dubai3 from '../../Assets/pdf/Dubai/Dubai 3.pdf'
import d1 from '../../Assets/pdf/Dubai/d1.webp'
import d2 from '../../Assets/pdf/Dubai/d2.webp'
import d3 from '../../Assets/pdf/Dubai/d3.webp'
import Bali1 from '../../Assets/pdf/Bali/Bali 1.pdf'
import Bali2 from '../../Assets/pdf/Bali/Bali 2.pdf'
import Bali3 from '../../Assets/pdf/Bali/Bali 3.pdf'
import Bali4 from '../../Assets/pdf/Bali/Bali 4.pdf'
import b1 from '../../Assets/pdf/Bali/b1.webp'
import b2 from '../../Assets/pdf/Bali/b2.webp'
import b3 from '../../Assets/pdf/Bali/b3.webp'
import b4 from '../../Assets/pdf/Bali/b4.webp'
import Kashmir1 from '../../Assets/pdf/Kashmir/kashmir 1.pdf'
import Kashmir2 from '../../Assets/pdf/Kashmir/kashmir 2.pdf'
import Kashmir3 from '../../Assets/pdf/Kashmir/kashmir 3.pdf'
import k1 from '../../Assets/pdf/Kashmir/k1.webp'
import k2 from '../../Assets/pdf/Kashmir/k2.webp'
import k3 from '../../Assets/pdf/Kashmir/k3.webp'
import Maldives1 from '../../Assets/pdf/Maldives/Maldives 1.pdf'
import Maldives2 from '../../Assets/pdf/Maldives/Maldives 2.pdf'
import Maldives3 from '../../Assets/pdf/Maldives/Maldives 3.pdf'
import Maldives4 from '../../Assets/pdf/Maldives/Maldives 4.pdf'
import Maldives5 from '../../Assets/pdf/Maldives/Maldives 5.pdf'
import m1 from '../../Assets/pdf/Maldives/m1.webp'
import m2 from '../../Assets/pdf/Maldives/m2.webp'
import m3 from '../../Assets/pdf/Maldives/m3.webp'
import m4 from '../../Assets/pdf/Maldives/m4.webp'
import m5 from '../../Assets/pdf/Maldives/m5.webp'
import Vietnam1 from '../../Assets/pdf/Vietnam/Vietnam 1.pdf'
import Vietnam2 from '../../Assets/pdf/Vietnam/Vietnam 2.pdf'
import Vietnam3 from '../../Assets/pdf/Vietnam/Vietnam 3.pdf'
import Vietnam4 from '../../Assets/pdf/Vietnam/Vietnam 4.pdf'
import v1 from '../../Assets/pdf/Vietnam/v1.webp'
import v2 from '../../Assets/pdf/Vietnam/v2.webp'
import v3 from '../../Assets/pdf/Vietnam/v3.webp'
import v4 from '../../Assets/pdf/Vietnam/v4.webp'
import Thailand1 from '../../Assets/pdf/Thailand/Thailand 1.pdf'
import Thailand2 from '../../Assets/pdf/Thailand/Thailand 2.pdf'
import Thailand3 from '../../Assets/pdf/Thailand/Thailand 3.pdf'
import t1 from '../../Assets/pdf/Thailand/t1.webp'
import t2 from '../../Assets/pdf/Thailand/t2.webp'
import t3 from '../../Assets/pdf/Thailand/t3.webp'
import Srilanka1 from '../../Assets/pdf/Srilanka/Srilanka 1.pdf'
import Srilanka2 from '../../Assets/pdf/Srilanka/Srilanka 2.pdf'
import Srilanka3 from '../../Assets/pdf/Srilanka/Srilanka 3.pdf'
import Srilanka4 from '../../Assets/pdf/Srilanka/Srilanka 4.pdf'
import sr1 from '../../Assets/pdf/Srilanka/sr1.webp'
import sr2 from '../../Assets/pdf/Srilanka/sr2.webp'
import sr3 from '../../Assets/pdf/Srilanka/sr3.webp'
import sr4 from '../../Assets/pdf/Srilanka/sr4.webp'
import Andaman1 from '../../Assets/pdf/Andaman/Andaman 1.pdf'
import Andaman2 from '../../Assets/pdf/Andaman/Andaman 2.pdf'
import Andaman3 from '../../Assets/pdf/Andaman/Andaman 3.pdf'
import Andaman4 from '../../Assets/pdf/Andaman/Andaman 4.pdf'
import a1 from '../../Assets/pdf/Andaman/a1.webp'
import a2 from '../../Assets/pdf/Andaman/a2.webp'
import a3 from '../../Assets/pdf/Andaman/a3.webp'
import g1 from '../../Assets/g1.webp'
import g2 from '../../Assets/g2.webp'
import g3 from '../../Assets/g3.webp'
import g4 from '../../Assets/g4.webp'
import g5 from '../../Assets/g5.webp'
import g6 from '../../Assets/g6.webp'
import g7 from '../../Assets/g7.webp'
import g8 from '../../Assets/g8.webp'
import g9 from '../../Assets/g9.webp'
import g10 from '../../Assets/g10.webp'
import g11 from '../../Assets/g11.webp'
import g12 from '../../Assets/g12.webp'
import g13 from '../../Assets/g13.webp'
import g14 from '../../Assets/g14.webp'
import g15 from '../../Assets/g15.webp'
import g16 from '../../Assets/g16.webp'
import g17 from '../../Assets/g17.webp'
import g18 from '../../Assets/g18.webp'
import g19 from '../../Assets/g19.webp'
import g20 from '../../Assets/g20.webp'
import g21 from '../../Assets/g21.webp'
import g22 from '../../Assets/g22.webp'

const data=[
    // Singapore  Start-------------------------------------------
    {
        imgsrc: s1,
        title: 'Singapore with Universal Studio, Sentosa & Night Safari',
        category: 'Singapore',
        hotelrating: '4',
        hotel: 'Mercure Tyrwhitt (hotel)',
        facilities:['Night Safari', 'Sentosa Magic', 'City Tour with Guide', 'Duck Tour', 'S.E.A. Aquarium', 'Singapore Zoo', 'Universal Studios'],
        document: singapore1,
    },
    {
        imgsrc: s2,
        title: 'Singapore with Universal Studio, Sentosa & Night Safari',
        category: 'Singapore',
        hotelrating: '4',
        hotel: 'The Boss (hotel)',
        facilities:['Gardens By The Bay and Marina Bay SkyPark', 'Night Safari', 'Sentosa Magic', 'City Tour with Guide (3 Hrs)', 'Universal Studios'],
        document: singapore2
    },
    {
        imgsrc: s3,
        title: 'Singapore with Universal Studio, Sentosa & Night Safari',
        category: 'Singapore',
        hotelrating: '4',
        hotel: 'Mercure Tyrwhitt (hotel)',
        facilities:['Sentosa Magic', 'City Tour with Guide', 'Duck Tour', 'S.E.A. Aquarium', 'Singapore Zoo', 'Night Safari',  'Universal Studios'],
        document: singapore3
    },
    {
        imgsrc: s4,
        title: 'Singapore with Universal Studio, Sentosa & Night Safari',
        category: 'Singapore',
        hotelrating: '4',
        hotel: 'Hotel Pullman Orchard Road', 
        facilities:['Singapore Zoo', 'Night Safari', 'Universal Studios with Meal Voucher',  'City Tour with Guide (3 Hrs)', 'S.EA. Aquarium'],
        document: singapore4
    },

    // Singapore End------------------------------------------

    // Dubai Start--------------------------------------------

    {
        imgsrc: d1,
        title: 'Dubai Trip with City Tour and Dhow Cruice',
        category: 'Dubai',
        hotelrating: '3',
        hotel: 'Versailles By Raviz Hotel', 
        facilities:['Dubai Trio - Deluxe (City Tour, Desert Safari, Dhow Creek Cruise) With Burj Khalifa 124th Floor Non- Prime Time', 'Half Day Dubai City Tour', 'Abu Dhabi City Tour With Ferrari World'],
        document: Dubai1
    },
    {
        imgsrc: d2,
        title: 'Dubai Trip with City Tour and Dhow Cruice',
        category: 'Dubai',
        hotelrating: '4',
        hotel: 'Hyatt Place Dubai Wasl District', 
        facilities:['Dubai Trio - Deluxe (City Tour, Desert Safari, Dhow Creek Cruise) With Burj Khalifa 124th Floor Non- Prime Time', 'Half Day Dubai City Tour', 'Museum Of Future With Transfers'],
        document: Dubai2
    },
    {
        imgsrc: d3,
        title: 'Dubai Trip with City Tour and Dhow Cruice',
        category: 'Dubai',
        hotelrating: '5',
        hotel: 'Crowne Plaza Jumeirah', 
        facilities:['Global Village and Miracle Garden with Transfers', 'Inside Burj Al Arab (Tour) With Transfers', 'Dubai Frame with Transfers', 'Sunset Yacht Tour SIC', 'Jet Ski (30 Mins) with Transfers'],
        document: Dubai3
    },

    // Dubai End---------------------------------------------------------

    // Bali Start-----------------------
    {
        imgsrc: b1,
        title: 'Nusa Penida West Tour And Full Day Tour of Watersports',
        category: 'Bali',
        hotelrating: '3',
        hotel: 'The Tusita Enlightsphere Hotel', 
        facilities:['Full Day tour of watersports', 'Padang Padang Beach', 'Uluwatu Temple', 'Nusa Penida West Tour B',  'Indonesian Lunch', '2-way Ferry'],
        document: Bali1
    },
    {
        imgsrc: b1,
        title: 'Nusa Penida West Tour And Full Day Tour of Watersports',
        category: 'Bali',
        hotelrating: '5',
        hotel: 'Crowne Plaza Jumeirah', 
        facilities:['Full Day tour of watersports', 'Padang Padang Beach', 'Uluwatu Temple', 'Nusa Penida West Tour B with Indonesian Lunch', '2-way Ferry'],
        document: Bali2
    },
    {
        imgsrc: b2,
        title: 'Nusa Penida West Tour And Full Day Tour of Watersports',
        category: 'Bali',
        hotelrating: '5',
        hotel: 'Anvaya Beach Hotel', 
        facilities:['Kintamani Tour', 'Jimbaran Beach ', 'Aristocat Beach Club Cruise', 'Buffet Lunch' , 'Snorkeling and Sailing to Nusa Lembongan from Bali', 'Hotel Transfer'],
        document: Bali3
    },
    {
        imgsrc: b3,
        title: 'Nusa Penida West Tour And Full Day Tour of Watersports',
        category: 'Bali',
        hotelrating: '5',
        hotel: 'Asvara Villa', 
        facilities:['Half Day Bali Swing with Nests and Lunch, Ubud', 'Watersports Package', 'Parasailing and Jet Ski in Tanjung Benoa', 'Bali Hai Sunset Dinner Cruise with Transfers', 'Tanjung Benoa'],
        document: Bali4
    },

    // Bali Done-------------------------

    // Kashmir Start------------------------
    {
        imgsrc: k1,
        title: 'Nusa Penida West Tour And Full Day Tour of Watersports',
        category: 'Kashmir',
        hotelrating: '5',
        hotel: 'Hotel Royal Milad', 
        facilities:['Srinagar Sightseeing', 'Shikara Ride', 'Day trip to Pahalgam'],
        document: Kashmir1
    },
    {
        imgsrc: k2,
        title: 'Nusa Penida West Tour And Full Day Tour of Watersports',
        category: 'Kashmir',
        hotelrating: '5',
        hotel: 'Hayat Residency', 
        facilities:['Zahgeer Continental', 'Shikara Ride', 'Mughal Gardens', 'ABC Valley Union Cab'],
        document: Kashmir2
    },
    {
        imgsrc: k3,
        title: 'Nusa Penida West Tour And Full Day Tour of Watersports',
        category: 'Kashmir',
        hotelrating: '5',
        hotel: 'Ennco resort', 
        facilities:['Anarkali Group of Houseboat', 'Royal Castle', 'Mughal Gardens'],
        document: Kashmir3
    },

    // Kashmir Complete --------------------------------------------------------------

    // Maldives Start---------------------------------------------------------------

    {
        imgsrc: m1,
        title: 'Trip to Maldives',
        category: 'Maldives',
        hotelrating: '5',
        hotel: 'Makunudu Island Trip To Maldives', 
        facilities:['Breakfast', 'Dinner', 'lunch'],
        document: Maldives1
    },
    {
        imgsrc: m2,
        title: 'Trip to Maldives',
        category: 'Maldives',
        hotelrating: '5',
        hotel: 'Medhufushi Island Resort Trip To Maldives', 
        facilities:['Bich Villa', 'Breakfast', 'lunch & Dinner'],
        document: Maldives2
    },
    {
        imgsrc: m3,
        title: 'Trip to Maldives',
        category: 'Maldives',
        hotelrating: '5',
        hotel: 'Villa Nautica Paradise Island Resort', 
        facilities:['Beach Villa', 'Breakfast', 'lunch', 'Dinner'],
        document: Maldives3
    },
    {
        imgsrc: m4,
        title: 'Trip to Maldives',
        category: 'Maldives',
        hotelrating: '5',
        hotel: 'Kaani Village and Spa', 
        facilities:['Double room with pool view', 'Breakfast', 'Lunch & Dinner'],
        document: Maldives4
    },
    {
        imgsrc: m5,
        title: 'Trip to Maldives',
        category: 'Maldives',
        hotelrating: '5',
        hotel: 'JW Marriott', 
        facilities:['Breakfast ', 'lunch', 'Overwater Villa Pool Sunrise'],
        document: Maldives5
    },

    // Maldives Done----------------------------------------------

    // Vietnam Start------------------------

    {
        imgsrc: v1,
        title: 'Vietnam Vista Retreat',
        category: 'Vietnam',
        hotelrating: '5',
        hotel: 'The Q Hotel', 
        facilities:['VICTORY CRUISES', 'Hanoi-Full Day City Tour', 'Day Trip to Ninh Binh (Hoa Lu - Tam Coc)'],
        document: Vietnam1
    },
    {
        imgsrc: v2,
        title: 'Vietnam Imperial Explorations',
        category: 'Vietnam',
        hotelrating: '5',
        hotel: 'Golden Holiday Hotel', 
        facilities:['Sunrise Central Hotel', 'Hanoi-Full Day City Tour', 'Full day Ho Chi Minh City Tour'],
        document: Vietnam2
    },
     {
        imgsrc: v3,
        title: 'Mystical Vietnam',
        category: 'Vietnam',
        hotelrating: '5',
        hotel: 'SOJO Hotel Ga Hanoi', 
        facilities:['Merry Land Hotel Da Nang', 'Hanoi - Full Day City Tour', ' Day Trip to Ninh Binh', 'Day trip to Bana Hill and Golden Bridge'],
        document: Vietnam3
    },
     {
        imgsrc: v4,
        title: 'Vietnam Adventura Abode',
        category: 'Vietnam',
        hotelrating: '5',
        hotel: 'Golden Holiday Hotel', 
        facilities:['Danang City Tour', 'Hoi An City Evening Tour with Lantern Boat Ride', 'Nguyen Gia Hotel', 'Hanoi-Full Day City Tour', 'Day Trip to Ninh Binh'],
        document: Vietnam4
    },

    // Vietnam Complete-----------------------
    
    // Thailand Start.........................
    {
        imgsrc: t1,
        title: 'Monday Escape to Thailand (Phuket-Krabi)',
        category: 'Thailand',
        hotelrating: '5',
        hotel: 'Bel Aire Patong', 
        facilities:['Coral Island tour with Lunch by Speed Boat (from Phuket)', 'Phi Phi Island Tour with Lunch by Jet Cruise', 'Mughal Gardens'],
        document: Thailand1
    },
    {
        imgsrc: t2,
        title: 'Phuket Krabi With Phi Phi Island, Krabi 4 Island And Full Day Jungle Tour',
        category: 'Thailand',
        hotelrating: '5',
        hotel: 'The Nature Phuket', 
        facilities:['Holiday Ao Nang Beach Resort', 'Phi Phi Island Tour with Lunch by Speed Boat', 'James Bond Island Tour'],
        document: Thailand2
    },
    {
        imgsrc: t3,
        title: 'Pattaya Bangkok With Coral Island Tour And Marine Park ',
        category: 'Thailand',
        hotelrating: '4',
        hotel: 'Mind Resort', 
        facilities:['Bangkok City Tour InterHotel Bangkok', 'Citrus Sukhumvit 11', 'Coral Island Tour with Lunch by Speed Boat'],
        document: Thailand3
    },

    // Thailand end------------------

    // Srilanka Start----------------------
    {
        imgsrc: sr1,
        title: 'Colombo',
        category: 'Srilanka',
        hotelrating: '5',
        hotel: 'Colombo', 
        facilities:['Colombo City Tour', 'Full Day Car at Disposal (80 KMs and 8 Hrs)'],
        document: Srilanka1
    },
    {
        imgsrc: sr2,
        title: 'Nusa Penida West Tour And Full Day Tour of Watersports',
        category: 'Srilanka',
        hotelrating: '5',
        hotel: 'JW Marriott', 
        facilities:['Anarkali Group of Houseboat', 'Royal Castle', 'Mughal Gardens'],
        document: Srilanka2
    },
    {
        imgsrc: sr3,
        title: 'Kandy, 2N Colombo',
        category: 'Srilanka',
        hotelrating: '5',
        hotel: 'Tourmaline Hotel', 
        facilities:['Full Day Car at Disposal', 'Ocean Colombo', 'Kandy City Tour ', 'Day Trip to Nuwara Eliya'],
        document: Srilanka3
    },
    {
        imgsrc: sr4,
        title: 'Kandy, 1N Nuwara Eliya, 2N Bentota, 1N Colombo',
        category: 'Srilanka',
        hotelrating: '5',
        hotel: 'The Golden Crown Hotel', 
        facilities:['Taj Bentota Resort & Spa', 'Taj Samudra', 'Bentota City Tour', 'Full Day Car at Disposal '],
        document: Srilanka4
    },

   // Srilanka end--------------

   // Andaman start---------------------
   {
    imgsrc: a1,
    title: '4N Serene Andaman Getaway',
    category: 'Andaman',
    hotelrating: '5',
    hotel: 'Lemon Tree Hotel', 
    facilities:['Aquays Hotel and Resort', 'Hotel Mansha Regency', 'Elephant Beach with Complementary Snorkelling'],
    document: Andaman1
},
{
    imgsrc: a2,
    title: 'Andaman Adventure Expedition',
    category: 'Andaman',
    hotelrating: '5',
    hotel: 'NK Havelock Eco Resort', 
    facilities:['Ross and North Bay Island', 'Tango Beach Resort', 'Megapode Resort', 'Elephant Beach with Complementary Snorkelling'],
    document: Andaman2
},
{
    imgsrc: a3,
    title: '6N Island Hopping Adventure',
    category: 'Andaman',
    hotelrating: '5',
    hotel: 'NK Havelock Eco Resort', 
    facilities:['Tango Beach Resort', 'Megapode Resort', 'Corbyns Cove Beach', 'Laxmanpur and Bharatpur Beach', 'Natural Coral Bridge Package'],
    document: Andaman3
},
{
    imgsrc: b4,
    title: 'Andaman Archipelago Escapade',
    category: 'Andaman',
    hotelrating: '5',
    hotel: 'Lemon Tree Hotel', 
    facilities:['Summer Sand Beach Resort', 'Hotel Mansha Regency', 'Corbyns Cove Beach', 'Ross and North Bay Island'],
    document: Andaman4
},

//---------- Goa Villas start-------------------------------------

{
    imgsrc: g1,
    title: '1 BHK LUXURY BELLA SERVICED APARTMENT @ CANDOLIM',
    category: 'villas',
    hotelrating: '4.6',
    hotel: 'Bella Serviced Apartment', 
    facilities:['Kitchen', 'Wifi', 'Free parking on premises', 'Dedicated workspace','Pool','Lift','TV','Air Conditioning'],
    link: 'https://www.airbnb.co.uk/rooms/46983434?source_impression_id=p3_1708161260_aOpb1%2FcR1%2BSqI2wk'
},
{
    imgsrc: g2,
    title: '2BHK Luxury Serviced Beach Apartment @ Calangute',
    category: 'villas',
    hotelrating: '4.6',
    hotel: 'Calangute', 
    facilities:['Smoking allowed', 'Gym','Free parking on premises', 'TV','Pool'],
    link: 'https://www.airbnb.co.uk/rooms/46982034?source_impression_id=p3_1708161743_tbPIlYdiLkp%2FqC88'
},
{
    imgsrc: g3,
    title: '2 BHK Luxury Serviced Apartment @ Calangute',
    category: 'villas',
    hotelrating: '4.6',
    hotel: 'Calangute', 
    facilities:['Kitchen', 'Wifi', 'Free parking on premises', 'Garden','Washing Machine','Dryer','Pool','Lift','TV','Air Conditioning'],
    link: 'https://www.airbnb.co.uk/rooms/908495435398144929?source_impression_id=p3_1708161743_UbshHK%2Bohort39iJ'
},
{
    imgsrc: g4,
    title: '2 BHK Silver Park Apartment At Calangute No.2(A)',
    category: 'villas',
    hotelrating: '4.6',
    hotel: 'Calangute', 
    facilities:['Kitchen', 'Wifi', 'Free parking on premises', 'Garden','Washing Machine','Dryer','Pool','Lift','TV','Air Conditioning'],
    link: 'https://www.airbnb.co.uk/rooms/25431972?source_impression_id=p3_1708161743_%2Bf0%2FQYguYjKuipEp'
},
{
    imgsrc: g5,
    title: '2 BHK Super Luxary Silver Park Apartment Near Beach',
    category: 'villas',
    hotelrating: '4.6',
    hotel: 'Silver Park serviced Apartments', 
    facilities:['Kitchen', 'Wifi','Washing Machine','Hair Dryer','Pool','Lift','TV','Air Conditioning'],
    link: 'https://www.airbnb.co.uk/rooms/39096552?source_impression_id=p3_1708161743_OVpuL5eWYHf42zbf'
},
{
    imgsrc: g6,
    title: '2 BHK Exotic Waves Luxury Apartment in Calangute',
    category: 'villas',
    hotelrating: '4.6',
    hotel: ' Calangute', 
    facilities:['Kitchen', 'Wifi','Free parking on premises','Hair Dryer','Pool','TV','Air Conditioning','Microwave'],
    link: 'https://www.airbnb.co.uk/rooms/24762097?source_impression_id=p3_1708161743_PIrQzwufK8708uMr'
},
{
    imgsrc: g7,
    title: '2 Luxurious 5 BHK Private Pool BEACH Villa @ BAGA',
    category: 'villas',
    hotelrating: '3.3',
    hotel: 'Baga', 
    facilities:['Beach access – Beachfront','Kitchen', 'Wifi','Free parking on premises','Private Pool','TV','Air Conditioning','Gym','Smoking allowed'],
    link: 'https://www.airbnb.co.uk/rooms/40217511?source_impression_id=p3_1708161743_k9cAAAybSVEyB5Ew'
},
{
    imgsrc: g8,
    title: '10 BHK Luxury Apartment @ Candolim For 20 Guest',
    category: 'villas',
    hotelrating: '5',
    hotel: 'Silver Park serviced Apartments', 
    facilities:['Luggage drop-off allowed','Dryer','Kitchen', 'Wifi','Free parking on premises','Pool','TV','Air Conditioning','Lift','Washing Machine'],
    link: 'https://www.airbnb.co.uk/rooms/38639720?source_impression_id=p3_1708161743_pjPV08%2B50c8a%2FaoQ'
},
{
    imgsrc: g9,
    title: '2 BHK Exotic Waves Luxury Apartment in Calangute-1',
    category: 'villas',
    hotelrating: '4.6',
    hotel: 'Silver Park serviced Apartments', 
    facilities:['Hair Dryer','Kitchen', 'Wifi','Free parking on premises','Pool','TV','Air Conditioning'],
    link: 'https://www.airbnb.co.uk/rooms/25468484?source_impression_id=p3_1708161743_1RSxKt7RCmG4jybp'
},
{
    imgsrc: g10,
    title: '2 BHK Silver Park Apartment At Calangute No.2(B)',
    category: 'villas',
    hotelrating: '4.7',
    hotel: 'Silver Park serviced Apartments', 
    facilities:['Hair Dryer','Kitchen', 'Wifi','Free parking on premises','Pool','TV','Air Conditioning','Lift','Washing Machine','Gym'],
    link: 'https://www.airbnb.co.uk/rooms/40218123?source_impression_id=p3_1708161743_%2FXte6pwF9DTAANhN'
},
{
    imgsrc: g11,
    title: '2 BHK Exotic Waves Luxury Apartment - Unit 2',
    category: 'villas',
    hotelrating: '4.6',
    hotel: 'Calangute', 
    facilities:['Hair Dryer','Kitchen', 'Wifi','Free parking on premises','Pool','TV','Air Conditioning'],
    link: 'https://www.airbnb.co.uk/rooms/25243096?source_impression_id=p3_1708161743_TDD3SsYvWZuD7U2B'
},
{
    imgsrc: g12,
    title: '4 BHK Private Pool Luxury Villa Marigold',
    category: 'villas',
    hotelrating: '4.6',
    hotel: 'Villa Marigold', 
    facilities:['Kitchen', 'Wifi','Free parking on premises','Pool','TV','Air Conditioning','Smoking allowed'],
    link: 'https://www.airbnb.co.uk/rooms/52157347?source_impression_id=p3_1708161743_gjh1N3EMWNzWrVaI'
},
{
    imgsrc: g13,
    title: '8 BHK Luxe Private Pool villa at Candolim',
    category: 'villas',
    hotelrating: '4.6',
    hotel: 'Candolim', 
    facilities:['Kitchen', 'Wifi','Free parking on premises','Pool','TV','Washing Machine','Air Conditioning','Smoking allowed'],
    link: 'https://www.airbnb.co.uk/rooms/988245055439993783?source_impression_id=p3_1708163854_HB45x4%2B%2BKdQyHsEd'
},
{
    imgsrc: g14,
    title: '4 BHK Boulevard villa in Goa',
    category: 'villas',
    hotelrating: '4.6',
    hotel: 'Baga', 
    facilities:['Kitchen', 'Wifi','Free parking on premises','Pool','TV','Washing Machine','Air Conditioning','Smoking allowed','Microwave'],
    link: 'https://www.airbnb.co.uk/rooms/967329493186796393?source_impression_id=p3_1708163854_tYH9HkWL8NzR%2BNsL'
},
{
    imgsrc: g15,
    title: '4 BHK Super Luxury Apartment @ Candolim',
    category: 'villas',
    hotelrating: '4.6',
    hotel: 'Silver Park serviced Apartments', 
    facilities:['Kitchen', 'Wifi','Free parking on premises','Pool','TV','Lift','Washing Machine','Air Conditioning','Dryer'],
    link: 'https://www.airbnb.co.uk/rooms/29982188?source_impression_id=p3_1708163854_o1U8ktTgCWcOS2ci'
},
{
    imgsrc: g16,
    title: '6 BHK Super Luxury Villa @ Baga',
    category: 'villas',
    hotelrating: '4.6',
    hotel: 'Baga', 
    facilities:['Kitchen', 'Wifi','Free parking on premises','Private Pool','TV','Washing Machine','Air Conditioning','Smoking allowed'],
    link: 'https://www.airbnb.co.uk/rooms/46840928?source_impression_id=p3_1708164328_lzDsxrXbe8HDGYnp'
},
{
    imgsrc: g17,
    title: '4 BHK Villa Orchid-Private Pool',
    category: 'villas',
    hotelrating: '4.6',
    hotel: 'Villa Orchid', 
    facilities:['Kitchen', 'Wifi','Free parking on premises','Pool','TV','Washing Machine','Air Conditioning','Smoking allowed'],
    link: 'https://www.airbnb.co.uk/rooms/907831734575822377?source_impression_id=p3_1708164328_%2BjO5MkvvGVlwb9pD'
},
{
    imgsrc: g18,
    title: '2 BHK Luxury Serviced Apartment @ Baga',
    category: 'villas',
    hotelrating: '4.6',
    hotel: 'Baga', 
    facilities:['Kitchen', 'Wifi','Dedicated workspace','Free parking on premises','Pool','TV','Air Conditioning','Lift'],
    link: 'https://www.airbnb.co.uk/rooms/47047646?source_impression_id=p3_1708164328_Yc0ul%2FGI95%2B5UMCy'
},
{
    imgsrc: g19,
    title: '3 BHK Luxury brand New Private PoolVilla @ Baga',
    category: 'villas',
    hotelrating: '4.6',
    hotel: 'Baga', 
    facilities:['Kitchen', 'Wifi','Free parking on premises','Pool','TV','Air Conditioning','Smoking allowed'],
    link: 'https://www.airbnb.co.uk/rooms/47842682?source_impression_id=p3_1708164328_YCoFygqkamKFj0mG'
},
{
    imgsrc: g20,
    title: '6 BHK Private Pool Beach Villa @ Baga',
    category: 'villas',
    hotelrating: '4.6',
    hotel: 'Baga', 
    facilities:['Kitchen', 'Wifi','Free parking on premises','Pool','TV','Air Conditioning','Smoking allowed','Private patio or balcony','Washing Machine'],
    link: 'https://www.airbnb.co.uk/rooms/46685605?source_impression_id=p3_1708164328_RuQCKJgcAta8mqkp'
},
{
    imgsrc: g21,
    title: '6 BHK Celebrity Ultra Luxury Villa at Baga',
    category: 'villas',
    hotelrating: '4.6',
    hotel: 'Baga', 
    facilities:['Kitchen', 'Wifi','Free parking on premises','Pool','TV','Air Conditioning','Smoking allowed','Microwave','Washing Machine'],
    link: 'https://www.airbnb.co.uk/rooms/47913801?source_impression_id=p3_1708164328_Njel745UH9%2FjFsBY'
},
{
    imgsrc: g22,
    title: '3 BHK Ultra Luxury Beach Villa @ Baga',
    category: 'villas',
    hotelrating: '4.6',
    hotel: 'Baga', 
    facilities:['Kitchen', 'Wifi','Dedicated workspace','Free parking on premises','Pool','TV','Air Conditioning','Hair Dryer','Washing Machine'],
    link: 'https://www.airbnb.co.uk/rooms/46935497?source_impression_id=p3_1708164328_FNwm9f%2B%2B6so9m9YC'
}
]

export default data;